...
const pageActions = require('../config/pageActions');
...
module.exports = function (app) {
	...
	app.get('/api/config/perspectives/:pname/dashboards/:dName/actions', function (req, res) {
		const perspective = pageActions.filter(p => p.id === req.params.pname)[0];
	...
	app.get('/api/config/components/:id/templates', function (req, res) {
		setTimeout(() => res.send(components[req.params.id]), respTime());
	});

	app.get('/api/config/:resourceName/fieldsets/:fieldSetId', function (req, res) {
		setTimeout(() => res.send(components[req.params.fieldSetId]), respTime());
	});
};
